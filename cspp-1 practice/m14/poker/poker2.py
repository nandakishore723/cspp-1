...
'''
D_CARDS = {'A':14, 'K':13, 'Q':12, 'J':11, 'T':10, '9':9, '8':8, '7':7, '6':6, \
 '5':5, '4':4, '3':3, '2':2}
def is_straight(hand):
    ...
    if all(True if c in '2345A' else False for c, s in hand):
        return True
    card_val = set(['--23456789TJQKA'.index(c) for c, s in hand])
    return len(card_val) == 5 and (max(card_val) - min(card_val) == 4)
def is_flush(hand):
    '''
...
def four_of_kind(hand):
    ...
    face_val = []
    for i in hand:
        face_val.append(D_CARDS[i[0]])
    face_val.sort()
    for k in range(len(face_val) - 1):
        if face_val[k+1] - face_val[k] == 0:
            count += 1
    return count == 3
